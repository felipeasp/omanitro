#!/usr/bin/env bash
# ==============================================================================
# install-privileged.sh - Privileged Installer for OmaNitro
#
# Dedicated privileged installation workflow for Acer Nitro 5 system assets:
...
#
# Security Hardening:
# 1. Enforces root privileges (EUID 0).
# 2. Creates an isolated root staging directory (0700 root:root via mktemp -d).
# 3. Stages assets into root staging FIRST without following symlinks and
#    rejecting all non-regular files (symlinks strictly forbidden).
# 4. Strictly validates SHA-256 integrity digests on root-staged files
#    BEFORE any system paths are touched.
# 5. Eliminates TOCTOU vulnerabilities: no user-writable paths are verified
#    and subsequently reopened. Destination files are installed directly
#    from the isolated, root-verified staging directory.
# ==============================================================================
set -euo pipefail
...
# Allow help without root execution
if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
  echo "Usage: sudo $0 [OPTIONS] [source-directory]"
  echo ""
  echo "Dedicated privileged installer for OmaNitro system components."
  echo ""
  echo "Options:"
  ...
  echo "  --commit <hash>   Specify commit hash for --download (default: 2ce11dc7582bfae1df7976e5d16cb99a9cf52538)"
...
if [[ $EUID -ne 0 ]]; then
  echo -e "${RED}Security Error: install-privileged.sh must be run as root.${NC}" >&2
  echo -e "Usage: sudo $0 [--local|--download] [source-directory]" >&2
  exit 1
fi

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# Pinned commit hash for remote authenticated archive retrieval
PINNED_COMMIT="2ce11dc7582bfae1df7976e5d16cb99a9cf52538"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      echo "  --local           Copy and verify assets from local repository (default)"
      echo "  --download        Download pinned commit archive directly into root staging"
      echo "  --commit <hash>   Specify commit hash for --download (default: ${PINNED_COMMIT})"
      echo "  -h, --help        Show this help message"
      exit 0
      ;;
    *)
...
if [[ "$DOWNLOAD_MODE" == "true" ]]; then
  ...
  if ! curl -fsSL "${REPO_URL}/archive/${PINNED_COMMIT}.tar.gz" -o "$ARCHIVE_PATH"; then
    echo -e "${RED}Security Error: Failed to download repository archive from authenticated source.${NC}" >&2
    exit 1
  fi
  ...
  # Move privileged files into staging root
  ...
  for rel_path in "${PRIVILEGED_ORDER[@]}"; do
    ...
    # Copy stream into root-owned staging directory FIRST without following symlinks
    cp --no-dereference -f "$src_file" "$staged_dest"
    chown root:root "$staged_dest"
    chmod 0600 "$staged_dest"

    # Validate staged file properties inside isolated root boundary
...
install_verified_asset() {
  ...
  # Atomic sanity check immediately before writing target
  local current_hash
